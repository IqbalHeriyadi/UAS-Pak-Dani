...
class Product extends Controller {
    ...
    public function index()
    {
        ...
        $paginationLinks = '<ul class="uk-pagination uk-margin-medium-top" data-uk-margin>';
        if ($totalPages > 1) {
            // Previous page link
            if ($currentPage > 1) {
                $paginationLinks .= '<li><a href="?page=' . ($currentPage - 1) . '"><span data-uk-pagination-previous></span></a></li>';
            } else {
                $paginationLinks .= '<li class="uk-disabled"><span data-uk-pagination-previous></span></li>';
            }

            // Numbered pages
            for ($i = 1; $i <= $totalPages; $i++) {
                if ($i == $currentPage) {
                    $paginationLinks .= '<li class="uk-active"><span>' . $i . '</span></li>';
                } else {
            ...
            if ($currentPage < $totalPages) {
                $paginationLinks .= '<li><a href="?page=' . ($currentPage + 1) . '"><span data-uk-pagination-next></span></a></li>';
            } else {
                $paginationLinks .= '<li class="uk-disabled"><span data-uk-pagination-next></span></li>';
            }
        }
        $paginationLinks .= '</ul>';
    ...
    public function detail($id)
    {
        ...
        $reviewsProduct = $productModel->getReviewsByProductId($id);
        $data['reviewsProduct'] = $reviewsProduct;
        $avg_rating = $productModel->getAverageRatingByProductId($id);
    ...
    public function addReview()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
            ...
            // Panggil model untuk menambahkan review
            $productModel = $this->model('Dashboard_product_model');
            $productModel->addReview($productId, $userId, $rating, $comment, $reviewDate);

            // Alihkan kembali ke halaman detail produk
            header('Location: ' . BASEURL . '/Product/detail/' . $productId);
            exit;
        }
    }
    ...
    {
        $data['judul'] = 'Detail Product';
        $historyModel = $this->model('Transaction_model');
        $data['history'] = $historyModel->paymentHistory();
        ...
        // $this->view('templates/header', $data);
        $this->view('product/history', $data);
        $this->view('templates/footer');
    }
