...
class Dashboard_product_model {
    ...
    {
        $query = "SELECT products.*, image.image AS image_filename, users.username
                FROM $this->table AS products
                LEFT JOIN $this->imageTable AS image ON products.id = image.product_id
        ...
        $this->db->query($query);
        $this->db->bind('id', $id);
        $result = $this->db->resultSet(); // Use resultSet() instead of single()
        ...
        $product_details = [
            ...
            'seller_username' => $result[0]['username'],
        ...
        if (isset($_FILES['images'])) {
            ...
            foreach ($images['tmp_name'] as $key => $tmp_name) {
                ...
                if ($images['error'][$key] === UPLOAD_ERR_OK) {
                    ...
                    $upload_dir = 'C:/xampp/htdocs/petshop/public/images/'; // Change the directory path as per your requirement
        ...
        if (isset($_FILES['images'])) {
            ...
            foreach ($images['tmp_name'] as $key => $tmp_name) {
                ...
                if ($images['error'][$key] === UPLOAD_ERR_OK) {
                    ...
                    $upload_dir = 'C:/xampp/htdocs/petshop/public/images/'; 
    ...
    public function addReview($productId, $userId, $rating, $comment, $reviewDate)
    {
        $query = "INSERT INTO reviews (product_id, user_id, rating, comment, review_date)
                VALUES (:product_id, :user_id, :rating, :comment, :review_date)";
        ...
        $this->db->bind('review_date', $reviewDate);
